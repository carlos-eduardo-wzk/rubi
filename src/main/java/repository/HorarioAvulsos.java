...
public class HorarioAvulsos implements Serializable {
	...
	public List<HorarioAvulso> carregarListaHorarioAvulsos() {
		return session.createQuery("from HorarioAvulso").list();
	}

	public HorarioAvulso porDataCola(java.util.Date date, Colaborador cola) {
		HorarioAvulso hav = (HorarioAvulso) session
				.createQuery("from HorarioAvulso where data_horario_inicial = :data and colaborador_id=:cola")
