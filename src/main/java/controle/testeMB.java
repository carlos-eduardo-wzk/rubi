...
public class testeMB implements Serializable {
	...
	public void recurMarcacaoADF() throws Exception {

		System.out.println("Sessao capturada  " + session.substring(12, 36));

		try {
			String str = "https://192.168.0.100:443/get_afd.fcgi?session=" +session.substring(12,36);
			URL url = new URL(str);
			HttpsURLConnection conn = (HttpsURLConnection) url.openConnection();
			conn.setRequestMethod("POST");
			conn.setRequestProperty("Content-type", "application/json");
			...
			OutputStream os = conn.getOutputStream();
			os.write("{\"day\":1,\"month\":1, \"year\":2005   }".getBytes());
	...
	public void recurDataHora() throws Exception {

		System.out.println("Sessao capturada  " + session.substring(12, 36));

		try {
			String str = "https://192.168.0.100:443/get_system_date_time.fcgi?session=" +session.substring(12,36);
			URL url = new URL(str);
			HttpsURLConnection conn = (HttpsURLConnection) url.openConnection();
			conn.setRequestMethod("POST");
			conn.setRequestProperty("Content-type", "application/json");
